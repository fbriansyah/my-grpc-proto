...
func (c *helloServiceClient) SayManyHello(ctx context.Context, in *HelloRequest, opts ...grpc.CallOption) (HelloService_SayManyHelloClient, error) {
	stream, err := c.cc.NewStream(ctx, &HelloService_ServiceDesc.Streams[0], HelloService_SayManyHello_FullMethodName, opts...)
	if err != nil {
		return nil, err
	}
	x := &helloServiceSayManyHelloClient{stream}
...
func _HelloService_SayManyHello_Handler(srv interface{}, stream grpc.ServerStream) error {
	...
	return srv.(HelloServiceServer).SayManyHello(m, &helloServiceSayManyHelloServer{stream})
}
